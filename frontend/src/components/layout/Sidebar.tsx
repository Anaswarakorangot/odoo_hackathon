...
export default function Sidebar() {
  ...
  // ── Determine ERP nav items based on role ────────────────────────────────
  const role = user?.role || 'owner';
  const meta = ROLE_META[role] || ROLE_META.owner;
  const itemKeys = ROLE_NAV[role] || ROLE_NAV.owner;
  const erpItems = itemKeys.map((k) => ALL_ITEMS[k]).filter(Boolean);
        ...
        <div>
          <div className="px-4 mb-2 flex items-center justify-between">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
              {isSystemAdmin ? 'ERP Modules' : meta.label}
            </h2>
          </div>
          <nav className="space-y-1 px-3">
            {erpItems.map((item) => (
              ...
              >
                <svg className={`w-5 h-5 ${item.path === window.location.pathname ? meta.textColor : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={item.icon} />
                </svg>
                {item.label}
              </NavLink>
